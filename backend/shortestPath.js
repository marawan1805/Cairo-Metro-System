        ...
        fs.readFile('metro_graph.json', 'utf8', (err, jsonString) => {
            ...
            for (let station in graph) {
                for (let connectedStation in graph[station]['connections']) {
                    let routeId = graph[station]['connections'][connectedStation]['route_id'];
            ...
            fs.writeFile('metro_graph_with_routes.json', JSON.stringify(graph), err => {
                if (err) {
...
function findShortestPath(graph, startNode, endNode) {
    ...
    for (let i = 0; i < path.length - 1; i++) {
        ...
        if (startIndex < endIndex) {
            ...
            routePoints = routesData[routeId].slice(endIndex, startIndex + 1).reverse();
        }
