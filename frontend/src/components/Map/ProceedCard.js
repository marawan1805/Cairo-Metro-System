...
const ProceedCard = ({ startStation, endStation, middleStations }) => {
  // Group middle stations by their routeId
  ...
  // Initial expanded state for each route
  ...
  // Calculate total time and estimated time for each stop
  const totalStations = middleStations.length + 1; // Include start and end stations
  const totalTime = totalStations * 3;
  const estimatedTimePerStop = totalTime / totalStations;
  var flag = 0;
  ...
  const groupEstimatedTimePerStop = groupTotalTime / numStops;
        return (
          <div className="station-card" key={routeId}>
            <div className="left-side">
              {routeId === "L3" && (
                <img src={L3} alt="Train" />
              )}
              {routeId === "L1" && (
                <img src={L1} alt="Train" />
              )}
              {routeId === "L2" && (
                <img src={L2} alt="Train" />
              )}
          ...
          <div className="divi">
            <svg width="10" height="70">
              <line
                ...
                strokeDasharray="2,10" // Change to strokeDasharray
              />
